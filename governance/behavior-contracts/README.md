...
1. **Copy Template**
   ```bash
   cp governance/behavior-contracts/core.contract_service.L1.yaml \
      governance/behavior-contracts/your.module.yaml
   ```
...
- **Ownership**: Owner field links to `governance/ownership-map.yaml`
- **Layers**: API patterns follow layer rules in `governance/architecture/layers-domains.yaml`
- **Policies**: Contracts are validated by `governance/policies/architecture-rules.yaml`
- **Health Metrics**: Coverage tracked in `governance/architecture-health.yaml`
