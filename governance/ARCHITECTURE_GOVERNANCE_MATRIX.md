...
4. **[Layers & Domains](#4-layers--domains--層級與領域)** - Semantic definitions for each layer
5. **[Roles & Capabilities](#5-roles--capabilities--模組角色與能力)** - What each module does
6. **[Behavior Contracts](#6-behavior-contracts--行為契約)** - Expected behaviors (API/events)
7. **[Lifecycle & Ownership](#7-lifecycle--ownership--生命週期與所有權)** - Responsibility and state
8. **[Policies & Constraints](#8-policies--constraints--策略與約束)** - Executable governance rules
9. **[Quality & Metrics](#9-quality--metrics--品質與指標)** - Health and evolution tracking
...
- **Rules**: `governance/policies/` - OPA/Rego policies
...
- **Primary**: `governance/architecture/layers-domains.yaml`
...
- [`governance/architecture/layers-domains.yaml`](./architecture/layers-domains.yaml)
...
- **Detailed**: `governance/modules/{module-id}.yaml`
...
- [`governance/modules/`](./modules/) (detailed specs)
...
- **Primary**: `governance/behavior-contracts/{module-id}.yaml`
...
- [`governance/behavior-contracts/`](./behavior-contracts/)
...
- **Primary**: `governance/ownership-map.yaml`
- **Inline**: Extended fields in `config/system-module-map.yaml`
...
- [`governance/ownership-map.yaml`](./ownership-map.yaml)
- [`config/system-module-map.yaml`](../config/system-module-map.yaml)
...
- **OPA Policies**: `governance/policies/architecture/*.rego`
- **YAML Rules**: `governance/policies/architecture-rules.yaml`
...
```rego
# governance/policies/architecture/layer-dependencies.rego
...
- **Metrics**: `governance/architecture-health.yaml`
...
- [`governance/architecture-health.yaml`](./architecture-health.yaml)
